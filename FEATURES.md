...
- ✅ **Place photos** — business photo strip **leads the sheet as a hero** (horizontally scrollable, at the top so it's visible at the peek height / in landscape); **tap a photo to open a full-screen, swipeable gallery** with a counter. The strip opens with the search response's ~10-photo preview (`[1][105][0][1][0][i][6][0]`), then **swaps in the full gallery (~40+)** pulled from the `batchexecute` **`hspqX`** RPC (`/MapsPhotoService.ListEntityPhotos`). That RPC turned out to need **no token at all** — not even a per-session `SNlM0e`/`at` — just the warmed session cookies, so it's exactly as keyless as reviews (no shared API key, nothing per-user to leak). Best-effort: an empty/failed fetch keeps the preview (no regression). Endpoint + request proto live in `calibration.json` (remotely updatable).
