...
- ✅ **Full reviews** — the place sheet's **Reviews tab** lists real reviews (author + photo, star rating, relative date, text) pulled from Google's keyless `listentitiesreviews` endpoint by feature id. **User-attached review photos** show as a Google-style thumbnail strip under each review (calibrated 2026-06-20: they hang under `review[12]`; `ReviewsParser` collects every image URL in that subtree — robust to 1-vs-N nesting — and the author photo at `[0][2]` stays out of it; unit-tested)
