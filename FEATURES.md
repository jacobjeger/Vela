...
- ✅ **Traversed route greys out** behind the vehicle (Google-style) — a line-progress gradient on the route (grey for the part driven, congestion-tinted ahead). **During nav the route line carries traffic, not the whole map** (changed 2026-06-19 per feedback — the `/maps/vt` whole-map overlay no longer auto-shows when navigating; it washed every road, and the ask was traffic on "just the road we're on". The route line still tints by overall congestion; the manual traffic toggle still paints the full map when you want it. True per-segment route colouring is the parity follow-up — see ROADMAP). Nav-overlay polish: speedometer + recenter lifted clear of the bottom bar, recenter is now an icon-only FAB tucked bottom-right. **Maneuver-banner swipe fixed** — `pointerInput(Unit)` had captured a stale step index so every swipe repeated one card; `rememberUpdatedState` makes each swipe walk to the next/prev step
...
- ✅ **Buildings** (footprints + 3-D massing) — OSM building data already in the Liberty tiles (`building` 2-D fill + `building-3d` extrusion at high zoom), given enough contrast + a subtle outline to read (was coloured almost identical to the land), and the 2-D `building` layer's minzoom lowered to 14 so **residential houses surface a zoom-level earlier** (~z17 now). **Keyless, no API key, no new data/infra.** *Caveat:* OpenFreeMap's keyless tiles drop small footprints from low-zoom tiles, so houses still only appear zoomed-in (Google shows them at neighbourhood zoom because its tiles are richer). Closing that gap needs richer tiles — **MapTiler** (has buildings, needs the key, path already wired but off) or our **own tiles baked from Microsoft US Building Footprints / Overture** (both free + ODbL, but bulk files → self-hosting infra)
