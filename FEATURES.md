...
- ✅ **Alternate routes** — Google's 2-3 driving alternates are surfaced + selectable in the directions panel (e.g. "20 min · via I-80 E" / "21 min · via Co Hwy E6"); each is drawn on the map along **real OSRM road geometry** (pulled with `alternatives=3` and paired by order), with a straight origin→dest fallback when the router returns fewer — **no more lines that double back on themselves** (the old scattered-point reconstruction is gone)
