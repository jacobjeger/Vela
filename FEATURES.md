...
- ✅ Tap a labelled POI **or a search-result pin** to open it; camera frames all results after a search. Tapping a POI also reads `name:latin`/`name:en` (not just `name`), and an **unnamed** POI icon (an apartment gym, an unnamed park/playground) **reverse-geocodes to a pin + address** instead of being a dead tap
...
- ✅ Clear-search (X) button; **results list opens tall (~half screen) and expands to nearly full-screen** (~94%) like Google's results page — drag the handle or tap the chevron up to expand, down to shrink, down again to hide it and browse the map (pins stay, a chip re-opens it); back gesture also hides it. **Swipe-down-from-anywhere dismiss** too (a nested-scroll handler, same shape as the place sheet): at the top of the list a downward drag collapses it to the chip, mid-list it scrolls — so the list that re-appears after dismissing a POI can itself be swiped away
