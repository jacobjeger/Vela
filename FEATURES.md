...
- ✅ **Place photos — keyless (preview always; full gallery intermittently, 2026-06).** The photo strip **leads the sheet as a hero** (horizontally scrollable; **tap → full-screen, swipeable gallery** with a counter). Photos come from the search response: the hero block at `[1][72][0][i][6][0]` *(Google moved it `[105]`→`[72]` on 2026-06-27, blanking every strip; hot-fixed via calibration `v7`)* **de-duped** (Google now serves the single hero photo **twice**), plus a small `[1][204][0][i][1][2][0][0]` "gallery preview" that **only big/landmark places carry** (a landmark → ~4; an ordinary business → **1**). The **full ~30-photo gallery is reachable keyless but INTERMITTENT** (corrected 2026-06-28 — *not* login-gated as first thought): the anonymous-WebView `hspqX` RPC answers with a degraded Street-View-only reply on some tries and the **real user gallery** on others (observed live — a place that showed only its preview populated the full gallery a few seconds later). `WebPhotoFetcher` now **retries** the same-origin fetch (up to 4×, taking the first non-empty set — same flake + cure as the reviews fetcher) so the gallery lands far more often; when every try comes back degraded it falls back to the de-duped preview. Posted-date in the viewer from `entry[21][6][8]`.
