...
- ✅ **Turn instructions keep the road name** ("Turn right **onto 13th St SE**", spoken + on the banner) — `DirectionsParser` tag-strips the `<step>` markup so the `<road>` rides along in the instruction, which both `NavSession.maneuverText` (banner) and `NavEngine` (voice) use verbatim. **Mock-nav regression test** `NavRoadNameTest` parses VERBATIM captured keyless markup and *drives* the route through `NavEngine`, asserting "onto Ave C" / "onto 13th St SE" are announced. **Bare turns now FILLED by reverse-geocode:** Google's keyless feed omits `<road>` on some turns (≈3 of 11 on a Lake Stevens route), leaving a bare "Turn left". `GoogleMapsDataSource.fillTurnRoads` reverse-geocodes (Nominatim, keyless/on-ethos — same source as long-press) a point just PAST each bare turn = the road being entered → "Turn left **onto** Elm St". Best-effort + **≤6 lookups/route done SEQUENTIALLY** (Nominatim forbids bulk/parallel — it rate-limits/blocks), de-duped across alternates; a miss leaves the bare turn. *(Caveat: it names the road from the maneuver's position, so an imperfect route geometry can mis-name a turn — under investigation.)*
